...
Writing $A$ and $B$ as depth-$k$ quantified formulas with quantifier prefixes of length $k$, consider each conjunct (e.g., $A \land \neg B$). This is obtained by concatenating the quantifier prefix of $A$ (a $\Sigma_k^p$ or $\Pi_k^p$ formula) with the prefix of $\neg B$ (which has the opposite leading quantifier). Concatenating these prefixes can increase the alternation depth by at most one, so each conjunct is expressible in $\Sigma_{k+1}^p$. A disjunction of $\Sigma_{k+1}^p$ predicates remains in $\Sigma_{k+1}^p$. Therefore the negation of the correctness condition lies in $\Sigma_{k+1}^p$, and the original universal property is in $\Pi_{k+1}^p$.
...
To see this is in $\Pi_2^p$: the biconditional $P \Leftrightarrow Q$ is equivalent to $(P \land Q) \lor (\neg P \land \neg Q)$. When $P = \exists w_F\, V_F(\cdot)$ (a $\Sigma_1^p$ predicate) and $Q = \exists w_G\, V_G(\cdot)$ (also $\Sigma_1^p$), each disjunct involves a conjunction of a $\Sigma_1^p$ predicate and a $\Pi_1^p$ predicate. Concatenating these quantifier prefixes yields a $\Sigma_2^p$ formula for each disjunct. A counterexample to correctness is therefore witnessed by $\exists y$ together with witnesses for one side and universal quantification over the other; hence the negation of the correctness condition lies in $\Sigma_2^p$, so <span class="sc">Correct-NP</span> is in $\Pi_2^p$. Hardness follows from Theorem 1, or directly: reduce from $\Pi_2$-QSAT $\forall u\, \exists v : \varphi(u, v)$ by setting $V_F(u, v) = \varphi(u, v)$, $V_G \equiv 1$, and $M = M_{\mathrm{id}}$.
